% u1' = -4*u1 - 2*u2 + cos(t) + 4*sin(t), u1(0) = 0
% u2' = 3*u1 + u2 - 3*sin(t) , u2(0) = -1
...
for t = t0:h:t1 %iterate through t=0 to t=2 with step-size h
    y1(end+1) = 2*exp(-t) -2*exp(-2*t) + sin(t); %append values to y array
    y2(end+1) = -3*exp(-t) +2*exp(-2*t);
end
...
u_1 = 2*exp(-t) -2*exp(-2.*t) + sin(t); 
u_2 = -3*exp(-t) +2*exp(-2.*t);
